  ...
  module Billing #:nodoc:
    class PaypalExpressGateway < Gateway
        ...
        xml.tag! 'SetExpressCheckoutReq', 'xmlns' => PAYPAL_NAMESPACE do
          xml.tag! 'SetExpressCheckoutRequest', 'xmlns:n2' => EBAY_NAMESPACE do
            ...
            xml.tag! 'n2:SetExpressCheckoutRequestDetails' do
              ...
              # NOTE: Order of fields must be respected or it could cause the request to be rejected
              # See: https://cms.paypal.com/us/cgi-bin/?&cmd=_render-content&content_ID=developer/e_howto_api_soap_r_SetExpressCheckout#id09BHC0QF07Q

              # OrderTotal is deprecated, this information is now provided
              # within the PaymentDetails tag instead
              #
              # DO NOT UNCOMMENT - This line is kept here for reference
              #xml.tag! 'n2:OrderTotal', order_total, 'currencyID' => options[:currency] || currency(money)
              ...
              xml.tag! 'n2:ReqConfirmShipping', options[:require_confirm_shipping] ? '1' : '0' # New
              xml.tag! 'n2:ReqBillingAddress', options[:require_billing_address] ? '1' : '0' # New
              xml.tag! 'n2:NoShipping', options[:no_shipping] ? '1' : '0'
              xml.tag! 'n2:AddressOverride', options[:address_override] ? '1' : '0'
              xml.tag! 'n2:LocaleCode', options[:locale] unless options[:locale].blank? # New
              xml.tag! 'n2:PaymentAction', action
              ...
              # Support for IPAddress seems to have been removed
              # from PayPal (not mentioned anywhere in most current API docs)
              #
              # DO NOT UNCOMMENT
      ...
      def add_payment_details(xml, money, options)
        currency_code = options[:currency] || currency(money)
        order_total = amount(money).to_f.zero? ? amount(100) : amount(money)

        xml.tag! 'n2:PaymentDetails' do # New
          ...
          if options[:items] && options[:items].respond_to?(:each)
            options[:items].each do |item|
              xml.tag! 'n2:PaymentDetailsItem' do
                xml.tag! 'n2:Name', item[:name] unless item[:name].blank?
                xml.tag! 'n2:Quantity', item[:quantity] unless item[:quantity].blank?
                xml.tag! 'n2:Description', item[:description] unless item[:description].blank?
                xml.tag! 'n2:Number', item[:number] unless item[:number].blank?
                ...
                xml.tag! 'n2:ItemURL', item[:url] unless item[:url].blank?
